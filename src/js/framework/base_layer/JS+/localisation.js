//Entity localisation functions
{
  function getHistoryFrameLocalisation (arg0_entity_id, arg1_history_frame, arg2_history_frame) {
    ...
    var old_land_area = getEntityArea(entity_id, old_history_frame.id);
    var old_options = old_history_frame.options;
    var new_land_area = getEntityArea(entity_id, new_history_frame.id);
    ...
    if (history_string.length == 0)
      history_string.push(`Nothing happened on this keyframe.`);

    //Return statement
    return `<tr timestamp = ${new_history_frame.id}>
