...
function switchTopbarTab (arg0_tab) {
  ...
  var left_sidebar_page_els = document.querySelectorAll(config.ui.left_sidebar_pages);
  var topbar_tab_button_els = document.querySelectorAll(config.ui.topbar_tab_buttons);
  ...
  for (var i = 0; i < topbar_tab_button_els.length; i++)
    if (topbar_tab_button_els[i].getAttribute("class") == tab)
      topbar_tab_button_els[i].setAttribute("class", `${topbar_tab_button_els[i]} active`);
